...
export default function ReportPage() {
  ...
  const stats: { label: string; value: number; alarm?: boolean }[] = [
    ...
    },
    {
      ...
      value: report.meta.criticalFindingCount ?? 0,
      alarm: (report.meta.criticalFindingCount ?? 0) > 0,
    },
  ];
              ...
              <p className="mt-2 rounded-sm border border-warning/40 bg-warning-dim px-3 py-2 text-sm text-warning">
                ...
                {incompletePasses(report).length > 0
                  ? ` — ${incompletePasses(report)
                      ...
                      .join(", ")} did not finish.`
                  : "."}{" "}
                Findings from finished passes are shown; missing passes are not a clean bill of health.
              </p>
            )}
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    {notApplicable ? (
                      <span className="text-xs font-medium text-faint">
                        N/A — not present in this harness
                      </span>
                    ) : c.findings.length === 0 ? (
